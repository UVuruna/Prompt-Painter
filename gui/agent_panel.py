...
What each switch OPENS INTO lives next door in
``gui/agent_subpanels.py`` (``AgentSubPanelsMixin``, audit
``docs/AUDIT-OOP-2026-08-18.md`` → R6): the five fine-tune builders and
the callbacks only their widgets use. This file keeps the panel itself
— the bands, the vars, Start/Pause/Stop, the settings round-trip and
the public readers (``force_aspect_ratio``, ``upscale_params``,
``pace_floats``).
...
than ``gui/__init__.py``) avoids a circular import; the sub-panel mixin
reads them from there for the same reason."""
...
from .agent_subpanels import AgentSubPanelsMixin
...
class AgentPanel(AgentSubPanelsMixin, ttk.Labelframe):

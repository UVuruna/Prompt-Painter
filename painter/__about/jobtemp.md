...
own multi-stage backups (BG → Crop → Aspect(force) → Upscale, plus the
Fixer AI's pre-fix snapshot). Passing no step keeps the EXACT
byte-for-byte path/behavior the four standalone tools have always
used — this is the CRITICAL regression guard, see "On-disk layout"
below. A named step is namespaced under its own subdir so a
multi-step pipeline can back up, and later restore to, each stage
independently without the steps ever colliding with each other or
with the unnamed backup.
...
## On-disk layout (the critical regression guard)
...
backup's path as if it were a real `rel` and try to restore it onto
`folder/__steps__/bg/a.png`, corrupting the live output tree. `clear()`
has no such guard — it wipes the WHOLE slot subdir (unnamed backups and
every named step together), which is the correct behavior for a full
job-close wipe.

## Connections

### Uses
- [Config (subfolder)](../config/___config.md) — `PROJECT_ROOT`,
  `JOBTEMP_DIRNAME`, `JOBTEMP_REMOVED_ALPHA`, `JOB_METRIC`,
  `JOBTEMP_STEPS_SUBDIR`, `JOBTEMP_STEP_NAMES`, `JOBTEMP_MAX_BYTES`

### Used by
- [GUI (folder)](../../gui/___gui.md) — `_start_tool` creates one
  `JobTemp` per tool job, the worker `backup`s each original, TIMES
  the op, then on `done` `measure`s / keeps the backup (else `drop`s
  it), the tool panel shows the metric + per-image time and restores
  one/all, and the app clears temps on panel CLOSE, on exit and at
  startup. All FOUR of today's call sites use the unnamed (step=None)
  form. The site-generation pipeline's own per-step backups and the
  Fixer AI's pre-fix snapshot are the `step=` form's callers.
- [Tests (folder)](../../tests/___tests.md) — `test_jobtemp.py`

## Classes

### JobTemp
`JobTemp(slot, folder)` — one tool job's backup store. The slot's
subdir is FRESH on construction (a reused slot never inherits an old
job's backups).
...
- `steps_for(rel) -> list[str]` — the named steps that currently hold a
  backup for `rel`, in PIPELINE order (`JOBTEMP_STEP_NAMES`). The
...
- `restore_all() -> int` — restore every UNNAMED backed-up file; returns
  the count. Named-step backups are out of scope (see "On-disk
  layout").
...
- `bytes_used` (property) `-> int` — cumulative size of every backup
  this job slot currently holds, summed from a per-`(rel, step)` size
  table so a repeated `backup()`/`drop()` of the same key never
  double-counts or drifts.
- `over_cap() -> bool` — `True` once `bytes_used` has reached
  `JOBTEMP_MAX_BYTES` (4 GiB). `JobTemp` NEVER auto-evicts anything —
  this is only a signal; a caller decides what to do.

## Functions

- `clear_all()` — wipe the WHOLE temp root — app-exit cleanup and a
  startup orphan sweep.
- `measure(kind, before, after) -> dict` — returns `{'before': 'WxH',
  'after': 'WxH', 'pct': float, 'label': str}`:
  ...
  - `aspect` — % growth of the CHANGED axis = the deformation (the
    ...
    reading would give 0% whenever the LARGER axis is stretched, so
    the changed axis is measured instead).

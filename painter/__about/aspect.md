...
original, so exactly ONE axis grows and neither is cut. See the flow
diagram for the exact formula.
...
### Uses
- [Config (subfolder)](../config/___config.md) — `ASPECT_TOL` (the
  already-at-ratio tolerance), `ASPECT_FILTER_OFF` /
...
### Used by
- [GUI (folder)](../../gui/___gui.md) — the standalone **"Aspect
  ratio…"** toolbar button (its `AspectRatioDialog` asks for `W : H` +
  an optional STACKED filter — a `FilterEditor`, GUI rework Phase 4,
  replacing this module's own scalar `filter_from`/`filter_to`/
  `filter_mode` params in the GUI's call — + a files-or-folder
  choice). The GUI now pre-filters WHICH FILES are passed to
  `change_aspect` itself, via [Shared Filter Framework](filters.md)'s
  `matches()`, so it always calls this function with its filter args
  at their unused `off` defaults; this function's OWN `filter_from`/
  `filter_to`/`filter_mode` parameters are unchanged and still
  exercised directly by `test_aspect.py` and the CLI. GUI rework
  Phase 5 additionally wires `reduced_ratio`/`decimal_ratio_label`
  into `AspectRatioCanvas`'s live dual label.
...
- `reduced_ratio(w, h) -> tuple[int, int]` (GUI rework Phase 5) — the
  ...
  h)` (1920×1080 -> `(16, 9)`). Raises `ValueError` loudly on a
  non-positive side rather than let `math.gcd`'s 0/negative handling
  silently produce a degenerate ratio.
...
- **`reduced_ratio`/`decimal_ratio_label` live here, not in the GUI**
  ...
  offline-testable engine layer; `AspectRatioCanvas` only calls them

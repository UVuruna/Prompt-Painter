...
**Flow:** [diagram](../__flow/filters.md)
...
**Wired in since Phase 4:** [GUI (folder)](../../gui/___gui.md)'s
`FilterEditor` widget edits a condition stack through this module's
`FilterCondition` / `condition_to_dict` / `condition_from_dict`, and
the standalone [Change Aspect Ratio](aspect.md) tool (`AspectRatioDialog`)
is its first caller — replacing the old scalar `ASPECT_FILTER_*`
dialog fields one-for-one, with a one-time settings migration for an
owner who already had a scalar filter saved.

**GUI rework Phase 6:** [Upscale](upscale.md)'s own four-field
aspect/size gate migrated too — each `AgentPanel` and the standalone
`UpscaleParamsDialog` now embed a `FilterEditor` (pre-seeded with one
Aspect (range) condition) beside a single min-side number, resolved
back into `upscale_if_small`'s unchanged kwargs by the GUI; a stacked
condition this resolution cannot express is still honored — never
silently dropped — via a direct `matches()` call at the two upscale
call sites. The BG/Crop tools stay on their pre-existing behaviour
until a later phase migrates them too.
...
when `lo <= min(width, height)` AND `max(width, height) <= hi`: a
portrait 800×1500 and a landscape 1500×800 are judged IDENTICALLY.
This is an AND of both extremes, not an OR of "either side
qualifies" — one outlier axis fails the whole condition even when the
other axis is comfortably in range. `WIDTH`/`HEIGHT` are the
orientation-SENSITIVE counterpart: they read one named dimension and
...
"exact 1:1" reliably matches any perfectly square image, but a pinned
...
`aspect.py`'s own `ASPECT_TOL` already uses.
...
### Used by
- [GUI (folder)](../../gui/___gui.md) — `FilterEditor` (the reusable
  stacked-condition widget), `AspectRatioDialog` (its first caller,
  GUI rework Phase 4), and — GUI rework Phase 6 — each `AgentPanel`'s
  upscale gate plus `UpscaleParamsDialog`. The BG/Crop tools are still
  unmigrated — a later phase.
...
- `matches(width: int, height: int, conditions: list[FilterCondition])
  ...
  matches everything.
- `condition_to_dict(condition) -> dict` / `condition_from_dict(data)
  -> FilterCondition` — the JSON-safe (de)serializers behind every
  place a condition STACK is persisted: `settings.json`'s
  `aspect_filter_conditions` key, and the shared preset library under
  `config.FILTER_PRESETS_SETTING` (`{name: [condition-dict, ...]}`).
  `condition_from_dict` raises `KeyError`/`TypeError`/`ValueError`
  loudly (root Rule #1) on a malformed dict rather than fabricating a
  condition from partial data.

## Design Decisions

- **AND stacking, not OR** (owner decision 2026-07-21): each condition
  further narrows what gets through — a single failing condition
  vetoes the whole image.
...
- **Polarity strings reuse the legacy wording exactly**
  ...
  so a future migration reads an old saved mode string straight
...
- **Kind/polarity strings ARE the GUI's future combo values**
  (Rule #4) — no separate ID-to-label table to keep in sync.
- **Loud on a bad kind/polarity.** An unrecognised string raises
  `ValueError` immediately (root Rule #1) rather than silently passing
  or failing every image.
- **The exact-aspect tolerance lives in the GUI layer, not here**
  (`config.FILTER_ASPECT_EXACT_TOL`, GUI rework Phase 4). This
  module's own "no hidden epsilon" design decision still holds
  literally — `matches()` never adds slop of its own; `FilterEditor`
  widens a single typed ratio into `[ratio - tol, ratio + tol]`
  BEFORE ever constructing the `FilterCondition`, so the engine still
  only ever sees a plain closed-interval containment test.
